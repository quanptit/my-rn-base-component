...
export interface ButtonProps extends ViewProps {
    ...
    isVertical?: boolean // ảnh với text sẽ được xắp xếp theo chiều dọc
...
export class Button extends PureComponent<ButtonProps> {
    ...
    private _renderTitle(textColor: string) {
        if (this.props.title == null) return null;
        return (
            <Text
                key={"title"}
                numberOfLines={1}
                style={[{color: textColor}, styles.textButton, this.props.textStyle]}
    ...
    private _renderImage(marginRightImg: number, marginTopImg: number) {
        ...
        if (imageProps) {
            let styleImage: any = {width: undefined, marginRight: marginRightImg, marginTop: marginTopImg};
            if (this.props.isVertical)
                styleImage.alignSelf = "stretch";
            return (
                <Image key={"image"}  {...imageProps}
                       style={[styleImage, imageProps.style]}/>
            );
        }
        return null
    }
    ...
    private _renderChildren(textColor: string) {
        let childElements = [];
    ...
    _renderInnerText(textColor: string) {
        if (this.props.isLoading) {
            return (
                <ActivityIndicator
                    ...
                    color={this.props.activityIndicatorColor || textColor}
                />
            )
        }
    ...
    render() {
        ...
        } else paddingStyle = styles.buttonPadding;

        if (this.props.disabled === true || this.props.isLoading === true) {
            return (
                <View
                    ref={(ref) => { this.rootV = ref}}
                    style={[{backgroundColor: backgroundColor}, styles.button, borderStyle, paddingStyle,
        ...
        if (Platform.OS === 'android') {
            ...
            return (
                <TouchableNativeFeedback
                    ...
                    <View
                        ref={(ref) => { this.rootV = ref }}
                        style={[styles.button, borderStyle, paddingStyle,
...
function getBtnColor(btnMode: ButtonModel, primaryColor: string): { backgroundColor: string, textColor: string } {
